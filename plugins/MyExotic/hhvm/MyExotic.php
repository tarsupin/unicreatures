...
/****** Plugin Variables ******/
	public static int $shopCreatures = 5;		// <int> The number of creatures to have in the exotic shop.
	public static int $daysGap = 4;				// <int> The number of days inbetween creatures leaving the shop.
	
	
/****** Get the list of exotic pets available in the shop ******/
	public static function getShopList (
	): array <int, array<str, mixed>>						// RETURNS <int:[str:mixed]> The data and list of the exotic shop creatures.
...
/****** Get data about a single exotic pet available in the shop ******/
	...
	(
		int $typeID		// <int> The type ID of the pet to get information on.
	): array <str, mixed>				// RETURNS <str:mixed> The data of the exotic pet in the shop.
...
/****** Check how many exotics are in the shop currently ******/
	...
	): int						// RETURNS <int> The number of creatures that are in the shop.
...
/****** Choose a random exotic to add to the shop ******/
	...
	): int					// RETURNS <int> the creature type ID to add to the shop.
	
	// $typeID = MyExotic::chooseForShop();
	{
		return (int) Database::selectValue("SELECT id FROM creatures_types WHERE rarity=? AND evolution_level=? ORDER BY RAND() LIMIT 1", array(20, 1));
	}
	
	
/****** Expire exotic pets from the shops ******/
	public static function expireShopPets (
	): bool					// RETURNS <bool> TRUE on successful expiration, FALSE on failure.
...
/****** Update the exotic pet list ******/
	...
	): bool					// RETURNS <bool> TRUE if the exotics were updated properly, FALSE on failure.
...
/****** Purchase exotic credits ******/
	...
	(
		int $uniID		// <int> The UniID that is purchasing the exotic credits.
	,	int $credits	// <int> The number of credits being purchased.
	): bool				// RETURNS <bool> TRUE on successful purchase, FALSE on failure.
...
/****** Spend an exotic credit on a creature ******/
	...
	(
		int $uniID			// <int> The UniID that purchased a creature with the exotic credit.
	,	int $creatureID		// <int> The creature ID that was purchased with this credit.
	): bool					// RETURNS <bool> TRUE on successful update, FALSE on failure.
...
/****** Count the user's available exotic credits ******/
	...
	(
		int $uniID		// <int> The UniID that is having their exotic credits counted.
	): int				// RETURNS <int> the number of available credits.
